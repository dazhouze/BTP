...
def evaluation(phase_0, phase_1, phase_pos, pos_index, read_queue, heter_snp, reg_s, reg_e, hit_p):
    ...
    for i in range(0, len(phase_0)): # phased region one by one
        ...
        for x in read_queue: # x is Read object
                ...
                if start <= k <= end: # heter-snp-marker is in read region
                    ...
                    if phase_0[row][col][1] is True: # phase 0 is significant
                        if snp_alt == phase_0[row][col][0]: # snp alt is phase 0
                            ...
                            conf_n += 1
                    elif phase_0[row][col][1] is not True: # phase 0 is not significant
                        if snp_alt == phase_0[row][col][0]: # snp alt is phase 0
                            cons_0_n += 1
                        elif snp_alt == phase_1[row][col][0]: # snp alt is phase 1
                            pass
                    '''Phase 1.'''
                    if phase_1[row][col][1] is True: # phase 1 is significant
                        if snp_alt == phase_1[row][col][0]: # snp alt is phase 1
                            cons_1_n += 1
                        elif snp_alt != phase_0[row][col][0]: # snp alt is other
                            conf_n += 1
                    elif phase_1[row][col][1] is not True: # phase 1 is not significant
                        if snp_alt == phase_0[row][col][0]: # snp alt is phase 0
                            pass
                        elif snp_alt == phase_1[row][col][0]: # snp alt is phase 1
                            cons_1_n += 1
            ...
            if (cons_0_n + cons_1_n + conf_n) > 4 or (cons_0_n + cons_1_n + conf_n) > 0.1*len(heter_snp):
