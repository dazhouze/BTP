...
def DetectSNP(chrom, reg_s, reg_e, input, read_queue):
    ...
    p = read_queue.add_first(myRead.Read('Begin', 0, 0, 0, None))
    ...
    for read in target:
        ...
        p = read_queue.add_after(p, myRead.Read(qname, start, end, ave_sq, snp))
    bamfile.close() # file handle closed
    read_queue.delete(read_queue.first()) # move first "begin" item, by test:
    assert read_queue.first().getNode().getElement().getQname() != 'Begin', \
